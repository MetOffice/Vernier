...
TEST(SystemTests, TimingTest)
{
  ...
  auto prof_main = meto::prof.start("MAIN");
  double t1 = omp_get_wtime();

  // Time a region
  {
    auto prof_sub = meto::prof.start("MAIN_SUB");
    sleep(1);
...
#pragma omp parallel
  {
    ...
    // Time nested region
    auto prof_sub2 = meto::prof.start("MAIN_SUB2");
    sleep(1);
    meto::prof.stop(prof_sub2);
  ...
  double prof_time  = meto::prof.get_total_walltime(prof_main, 0);
