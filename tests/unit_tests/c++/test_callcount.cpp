...
{
  // Start main region
  auto prof_main = meto::prof.start("MainRegion");
...
#pragma omp parallel default(none) shared(prof_sub_shared, meto::prof, num_threads)
  {
...
#pragma omp single
    ...
    {
      prof_sub_private = meto::prof.start("SubRegion");
      meto::prof.stop(prof_sub_private);
    }
...
#pragma omp critical
  ...
  for (int thread = 0; thread < num_threads; ++thread)
  {
    ...
    EXPECT_EQ(meto::prof.get_call_count(hash,thread), num_threads-thread);

    int incr = (thread==0) ? 1 : 0;
    EXPECT_EQ(meto::prof.get_prof_call_count(thread), num_threads-thread+incr);
  }
