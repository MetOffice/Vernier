...
class TestVernierReader(unittest.TestCase):
    ...
    def test_load_threaded_data_from_directory(self):
        ...
        self.assertEqual(loaded_data.data["FULL"].n_calls, [1, 1])
        self.assertEqual(loaded_data.data["FULL"].time_percent, [40.0, 40.031])
        self.assertEqual(loaded_data.data["FULL"].self_time, [2.0, 2.004])
        self.assertEqual(loaded_data.data["FULL"].total_time, [5.0, 5.006])
        self.assertEqual(loaded_data.data["FULL"].cumul_time, [4.0, 2.004])
        self.assertEqual(loaded_data.data["FULL"].rank, [0, 1])
        self.assertEqual(loaded_data.data["FULL"].thread, [0, 0])
        self.assertEqual(loaded_data.data["MAIN_SUB"].n_calls, [2, 1, 1, 1, 2, 1, 1, 1])
        self.assertEqual(loaded_data.data["MAIN_SUB"].time_percent, [40.0, 20.0, 20.0, 20.0, 39.956, 19.978, 19.978, 19.977])
        self.assertEqual(loaded_data.data["MAIN_SUB"].self_time, [2.0, 1.0, 1.0, 1.0, 2.0, 1.0, 1.0, 1.0])
        self.assertEqual(loaded_data.data["MAIN_SUB"].total_time, [3.0, 2.0, 2.0, 2.0, 3.002, 2.004, 2.002, 2.0])
        self.assertEqual(loaded_data.data["MAIN_SUB"].cumul_time, [2.0, 6.0, 10.001, 11.001, 4.004, 9.012, 10.012, 11.012])
        self.assertEqual(loaded_data.data["MAIN_SUB"].rank, [0, 0, 0, 0, 1, 1, 1, 1])
        self.assertEqual(loaded_data.data["MAIN_SUB"].thread, [0, 3, 2, 1, 0, 1, 3, 2])
        self.assertEqual(loaded_data.data["MAIN_SUB2"].n_calls, [1, 1, 1, 1, 1, 1, 1, 1])
        self.assertEqual(loaded_data.data["MAIN_SUB2"].time_percent, [20.0, 20.0, 20.0, 20.0, 20.056, 20.017, 20.012, 19.979])
        self.assertEqual(loaded_data.data["MAIN_SUB2"].self_time, [1.0, 1.0, 1.0, 1.0, 1.004, 1.002, 1.002, 1.0])
        self.assertEqual(loaded_data.data["MAIN_SUB2"].total_time, [1.0, 1.0, 1.0, 1.0, 1.004, 1.002, 1.002, 1.0])
        self.assertEqual(loaded_data.data["MAIN_SUB2"].cumul_time, [5.0, 7.001, 8.001, 9.001, 5.008, 6.01, 7.012, 8.012])
        self.assertEqual(loaded_data.data["MAIN_SUB2"].rank, [0, 0, 0, 0, 1, 1, 1, 1])
        self.assertEqual(loaded_data.data["MAIN_SUB2"].thread, [3, 0, 2, 1, 1, 3, 0, 2])
